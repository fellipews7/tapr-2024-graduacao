...
namespace TesteLabs.Controllers
{
    ...
    public class AulaController : ControllerBase
    {
        ...
        public AulaController(IAulaService corpoAppService, IMapper mapper)
        {
        ...
        [HttpGet("{id:int}")]
        public async Task<ActionResult<AulaViewModel>> GetById(Guid id)
        {
            try
            {
                return _mapper.Map<AulaViewModel?>(await _service.FirstOrDefaultAsync(x => x.Id == id));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public async Task<ActionResult> Post(AulaViewModel request)
        {
            try
            {
        ...
        [HttpPut]
        public async Task<ActionResult> Put(AulaViewModel request)
        {
            try
            {
                var entity = await _service.FirstOrDefaultAsync(x => x.Id == request.Id) ?? throw new Exception("Não foi encontrado nenhum registro para exclusão.");
                ...
                return Ok("Aula atualizado com sucesso.");
            }
            catch (Exception ex)
            {
                return StatusCode(StatusCodes.Status400BadRequest,
